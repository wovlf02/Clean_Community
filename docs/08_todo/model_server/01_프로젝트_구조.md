# 01. FastAPI 프로젝트 구조

**관련 문서**: [개발 환경 설정](../../06_development/setup.md) | [시스템 설계](../../03_architecture/system-design.md)

---

## 📋 개요

이 문서는 감정분석 AI 모델 서버의 FastAPI 프로젝트 디렉토리 구조를 정의합니다.

---

## ✅ 체크리스트

- [ ] 프로젝트 디렉토리 생성
- [ ] 기본 파일 구조 설정
- [ ] `__init__.py` 파일 생성
- [ ] `.gitignore` 설정
- [ ] 환경 변수 파일 구조화

---

## 📁 디렉토리 구조

> **변경**: `apps/ai-server/` → `ai_server/` (프로젝트 루트 기준)

```
ai_server/
├── app/
│   ├── __init__.py
│   ├── main.py                 # FastAPI 앱 엔트리포인트
│   ├── config.py               # 설정 및 환경 변수
│   │
│   ├── api/
│   │   ├── __init__.py
│   │   └── routes/
│   │       ├── __init__.py
│   │       ├── analyze.py      # 감정분석 API 라우트
│   │       └── health.py       # 헬스체크 라우트
│   │
│   ├── models/
│   │   ├── __init__.py
│   │   ├── classifier.py       # MultiLabelClassifier 정의
│   │   └── ensemble.py         # 앙상블 모델 래퍼
│   │
│   ├── schemas/
│   │   ├── __init__.py
│   │   ├── request.py          # 요청 스키마 (Pydantic)
│   │   └── response.py         # 응답 스키마 (Pydantic)
│   │
│   ├── services/
│   │   ├── __init__.py
│   │   └── model_loader.py     # 모델 로딩 서비스
│   │
│   └── utils/
│       ├── __init__.py
│       ├── text_processor.py   # 텍스트 전처리
│       └── constants.py        # 상수 정의 (라벨명 등)
│
├── tests/
│   ├── __init__.py
│   └── conftest.py             # pytest 설정
│
├── .env.example                # 환경 변수 예시
├── .gitignore
├── Dockerfile
├── requirements.txt
├── requirements-dev.txt        # 개발 의존성
└── README.md
```

> **모델 파일 위치**: 모델 파일(.pt)은 `ai-model/models/` 폴더에 위치합니다.

---

## 📄 주요 파일 설명

### app/main.py

FastAPI 애플리케이션의 메인 엔트리포인트입니다.

```python
from fastapi import FastAPI
from contextlib import asynccontextmanager
from app.api.routes import analyze, health
from app.services.model_loader import load_models

@asynccontextmanager
async def lifespan(app: FastAPI):
    # 시작 시 모델 로드
    load_models()
    yield
    # 종료 시 정리

app = FastAPI(
    title="Emotion Analysis API",
    description="한국어 혐오 표현 탐지 AI API",
    version="1.0.0",
    lifespan=lifespan
)

app.include_router(analyze.router, prefix="/analyze", tags=["분석"])
app.include_router(health.router, tags=["헬스체크"])
```

### app/config.py

환경 변수 및 설정을 관리합니다.

```python
from pydantic_settings import BaseSettings

class Settings(BaseSettings):
    # 서버 설정
    HOST: str = "0.0.0.0"
    PORT: int = 8000
    DEBUG: bool = False
    
    # 모델 설정
    MODEL_PATH: str = "./models"
    KCELECTRA_MODEL: str = "beomi/KcELECTRA-base"
    SOONGSIL_MODEL: str = "soongsil-ai/soongsil-bert-base"
    ROBERTA_MODEL: str = "klue/roberta-base"
    
    # 앙상블 가중치
    KCELECTRA_WEIGHT: float = 0.35
    SOONGSIL_WEIGHT: float = 0.33
    ROBERTA_WEIGHT: float = 0.32
    
    # 추론 설정
    MAX_LENGTH: int = 128
    BATCH_SIZE: int = 32
    
    class Config:
        env_file = ".env"

settings = Settings()
```

### app/utils/constants.py

상수 및 라벨 정의입니다.

```python
# 9개 혐오 카테고리 라벨
LABELS = [
    "여성/가족",
    "남성", 
    "성소수자",
    "인종/국적",
    "연령",
    "지역",
    "종교",
    "기타 혐오",
    "악플/욕설"
]

# 클래스별 최적화된 임계값
THRESHOLDS = {
    "여성/가족": 0.50,
    "남성": 0.50,
    "성소수자": 0.53,
    "인종/국적": 0.29,
    "연령": 0.50,
    "지역": 0.50,
    "종교": 0.39,
    "기타 혐오": 0.50,
    "악플/욕설": 0.45
}

# 모델 가중치
MODEL_WEIGHTS = {
    "kcelectra": 0.35,
    "soongsil": 0.33,
    "roberta": 0.32
}
```

---

## 🔗 참고 문서

| 문서 | 경로 | 설명 |
|------|------|------|
| 개발 환경 설정 | `docs/06_development/setup.md` | 전체 프로젝트 구조 |
| 모델 아키텍처 | `ai-model/docs/03_모델_아키텍처.md` | 모델 구조 및 추론 로직 |
| 프로젝트 개요 | `ai-model/docs/01_프로젝트_개요.md` | 프로젝트 구조 참고 |

---

**다음 문서**: [02_환경_설정.md](./02_환경_설정.md)
